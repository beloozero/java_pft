  ...
  public void verifyContactListInUI() {
    if (Boolean.getBoolean("verifyUI")) {
      ...
      assertThat(uiContacts, equalTo(dbContacts.stream()
              .map((c) -> c.withAllPhones(app.contact().mergePhones(c)).withAllEmail(app.contact().mergeEmail(c)))
              .map((c) -> new ContactData().withId(c.getId()).withFirstName(c.getFirstName()).withLastName(c.getLastName())
                      .withAddress(c.getAddress()).withAllPhones(c.getAllPhones()).withAllEmail(c.getAllEmail()))
              .collect(Collectors.toSet())));
    }
  }
}
